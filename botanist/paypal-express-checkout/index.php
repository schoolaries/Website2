...
if(!isset($_SERVER['HTTPS'])){
	header("Location: https://localhost/botanist/paypal-express-checkout/index.php");
	die();
}
...
if(isset($_SESSION["cart_products"])) //Post Data received from product list page.
{
	...
	$i = 0;
		...
		$results = $mysqli->query("SELECT ProductName, ProductDesc, Price FROM products WHERE ProductCode='$product_code' LIMIT 1");
		$obj = $results->fetch_object();
		
        $paypal_data .= '&L_PAYMENTREQUEST_0_NAME'.$i.'='.urlencode($obj->ProductName);
		...
		$paypal_product['items'][] = array('itm_name'=>$obj->ProductName,
											'itm_price'=>$obj->Price,
			...
			echo 'Your Transaction ID : '.urldecode($httpParsedResponseAr["PAYMENTINFO_0_TRANSACTIONID"]);
				...
				*/
				...
				if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
				{
					...
					*/
					$UserID = test_input($_SESSION['LogID']);
					$Cost = test_input($_SESSION['Cost']);
					echo $UserID;
					echo "<br";
					echo $Cost;
					echo "<br";
					
					$query = $con1->prepare ( "INSERT INTO `order`(`UserID`, `Cost`) VALUES (?,?)" );
					$query->bind_param('ss', $UserID, $Cost);
					if ($query->execute()) {
