...
if(!isset($_SERVER['HTTPS'])){ 
	header("Location: https://localhost/botanist/2fa2.php");
	die();
}
...
//No error
	...
	if(isset($_POST['submit'])){
		...
		$tenmins = 600; 
		if($keyage > $tenmins) {
			echo "<br> Verification code is more than 10 minutes old!";
			$error = "true";
		}
		if ($error == "true" || $error1 == "true") {
			echo "Error authenticating the code";
		}
		else {
			//manageProduct.php set session check to false then redirect
			$_SESSION['ManageProduct'] = "True";
			$_SESSION['2fa'] = "Succ";
